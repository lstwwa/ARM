...
void display(char *jpg)
{
	...
	int m = lcd_w<cinfo.output_width ? lcd_w: cinfo.output_width;	
	int n = lcd_h<cinfo.output_height? lcd_h: cinfo.output_height;
	
	
	if(m==cinfo.output_width || n==cinfo.output_height)
	{
		...
		w = (lcd_w-x)<cinfo.output_width ? (lcd_w-x) : cinfo.output_width;
		h = (lcd_h-y)<cinfo.output_height? (lcd_h-y) : cinfo.output_height;
	...
	//if(m==lcd_w || n==lcd_h)
	{
		printf("!!!!!!!!!!!!!!!大图片!!!!!!!!!!!!!!!!!!!\n");
		printf("m=%d,n=%d\n",m,n);
		x=cinfo.output_width%m;
		y=cinfo.output_height%n;
		printf("x=%d,y=%d\n",x,y);
		w=lcd_w/x;
		h=lcd_h/y;
		printf("w=%d,h=%d\n",w,h);
	//	lcdtmp +=  (y*lcd_w+ x)*(lcd_bpp/8);
		...
		for(int j=0; j<480; j++)
		{
			...
			lcdtmp = lcdtmp + (lcd_w*lcd_bpp/8); // lcd显存指针向下偏移一行
			rgbtmp = rgbtmp + (row_stride); // rgb指针向下偏移一行
		}		
